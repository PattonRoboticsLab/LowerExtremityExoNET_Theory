...
% EXONET
EXONET.nJoints = 22;    % 11 = only ankle-toe springs
                        % 22 = only knee-toe springs
                        % 2 = both ankle-toe and knee-toe springs
EXONET.nParameters = 3; % number of parameters for each spring
EXONET.nElements = 1;   % number of stacked elements per joint
EXONET.K = 1000;        % springs stiffness in [N/m]
global TENSION
%TENSION = @(L0,L)   (EXONET.K.*(L-L0)).*((L-L0)>0).*((L*L0)>0); % Linear Tension Springs
TENSION = @(L0,L)   (EXONET.K.*(L-L0)).*((L-L0)<0).*((L*L0)>0); % Linear Compression Springs
...
%TAUsDESIRED = [ankle_moment.*(-1).*BODY.Mass, knee_moment.*BODY.Mass]; % moments of force in [Nm]
...
TAUs = zeros(length(PHIs),2,length(Grid));
markerSize = 4;
lineWidth = 1;
...
for i = 1:length(Grid)
    ...
    TAUs(:,:,i) = exoNetTorquesLeg(pp,PHIs,EXONET,BODY);
...
TAUsBest = exoNetTorquesLeg(p,PHIs,EXONET,BODY);
plot(TAUsBest(:,1),TAUsBest(:,2),'^-','LineWidth',lineWidth,'Color','b','MarkerSize',markerSize,'MarkerFaceColor','b')


% PLOT DESIRED TORQUE
plot(TAUsDESIRED(:,1),TAUsDESIRED(:,2),'^-','LineWidth',lineWidth,'Color','r','MarkerSize',markerSize,'MarkerFaceColor','r')
...
%% SCORE
ankleVar = zeros(length(PHIs),length(colors));
kneeErr = zeros(length(PHIs),length(colors));
kneeSum = zeros(length(PHIs),length(colors));
for t = 1:length(PHIs)
    o = 1;
    for i = 1:length(Xgrid):length(Grid)
    ...
    kneeErr(t,o) = sum((TAUs(t,2,i:i+length(Xgrid)-1)-TAUsDESIRED(t,2)).^2);
