...
for i = 1:size(PHIs,1)
    ...
    for element = 1:EXONET.nElements
        ...
        if plotIt
            plot(EXONET.Tdist(i,1,element),EXONET.T(i,1,element),'o','MarkerSize',7,'MarkerFaceColor',ColorsS(1,:),'MarkerEdgeColor','w')
            xlabel('L [m]')
            ylabel('Tension [N]')
    ...
    for element = 1:EXONET.nElements
        ...
        if plotIt
            plot(EXONET.Tdist(i,2,element),EXONET.T(i,2,element),'o','MarkerSize',7,'MarkerFaceColor',ColorsS(2,:),'MarkerEdgeColor','w')
        end
        hold on
        tau = tau + new_tau; % + element's torque
    end
    ...
    if EXONET.nJoints == 3
        ...
        for element = 1:EXONET.nElements
            ...
            if plotIt
                plot(EXONET.Tdist(i,3,element),EXONET.T(i,3,element),'o','MarkerSize',7,'MarkerFaceColor',ColorsS(3,:),'MarkerEdgeColor','w')
                box off
            end
